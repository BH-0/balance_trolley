...
volatile u8 RF2G4_Receive_Data[14] = { 0 };	// 2.4G接收数据缓存数组
...
u8 RF2G4_Write_Reg(u8 Reg,u8 Write_Value)
{
	u8 V_Reg;	
  	...
  	V_Reg =SPI1_ReadWriteByte(Reg);		// 发送寄存器号

    SPI1_ReadWriteByte(Write_Value);	// 写入寄存器的值
...
u8 RF2G4_Write_Cont(u8 Reg, u8* P_Data, u8 N_Data)
{
	...
	u8 C_Write = 0;
 	...
 	RF2G4_CSN = 0;          			// 使能SPI传输
  	...
  	V_Reg = SPI1_ReadWriteByte(Reg);	// 选定寄存器,并读取状态值
	
  	for(; C_Write<N_Data; C_Write++)
	{
        SPI1_ReadWriteByte(*P_Data++); 	// 写入数据
	}
...
u8 RF2G4_Read_Reg(u8 Reg)
{
    ...
    SPI1_ReadWriteByte(Reg);   			// 发送寄存器号
  	...
  	V_Reg=SPI1_ReadWriteByte(0xFF);		// 读取寄存器内容
...
u8 RF2G4_Read_Cont(u8 Reg, u8* P_Data, u8 N_Data)
{
	...
	u8 C_Read = 0;	
  	...
  	V_Reg=SPI1_ReadWriteByte(Reg);		// 发送寄存器值(位置),并读取状态值
	
 	for(; C_Read<N_Data;C_Read++)
	
	P_Data[C_Read] = SPI1_ReadWriteByte(0xFF);	// 读出数据
...
u8 RF2G4_Check(void) {
    ...
    SPI1_SetSpeed(SPI_BAUDRATEPRESCALER_4);            // SPI速度为9Mhz（SI24R1的最大SPI时钟为10Mhz）
