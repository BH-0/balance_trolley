...
void MX_USART1_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART1_Init 2 */
    //HAL_UART_Receive_IT(&huart1, data, 1);  //开启接收中断
...
void MX_USART2_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART2_Init 2 */
    //HAL_UART_Receive_IT(&huart2, data, 1);  //开启接收中断
...
{
    HAL_UART_Transmit(&huart1,(const uint8_t *)cmd,3,50);
}
...
//
//    ProcessData(huart);//接收到后马上就调用处理函数
//
//    StartUartRxDMA(&huart1);
//
//}
...
void ProcessData(UART_HandleTypeDef *huart)
{
    if(huart == &huart1)
    {
        //得到已经接收了多少个字节 = 总共要接收的字节数 - NDTR
        RxLen = UART_RX_BUF_SIZE - huart1.hdmarx->Instance->NDTR;
        if(RxLen > 0)//收到不为空
        {
            //收到后的处理代码
            JY61_res_bit = 1;
    ...
    {
        //得到已经接收了多少个字节 = 总共要接收的字节数 - NDTR
        RxLen2 = UART_RX_BUF_SIZE - huart2.hdmarx->Instance->NDTR;
        if(RxLen2 > 0)//收到不为空字节
        {
            //收到后的处理代码
...
//启动DMA接收
char UartRxData(UART_HandleTypeDef *huart, unsigned char *buf, const unsigned int len)
{
    HAL_StatusTypeDef status;
    char ret = TRUE;

    status = HAL_UART_Receive_DMA(huart, (unsigned char*)buf, len);
    if(HAL_OK != status)
    {
        ret = FALSE;
    }
...
//启动DMA发送
char UartTxData(UART_HandleTypeDef *huart, unsigned char *buf, const unsigned int len)
{
    HAL_StatusTypeDef status;
    char ret = TRUE;
    ...
    if(huart == &huart1)
    {
        if(sendCompleteSign == FALSE || len == 0)
        {
            return FALSE;
        }
    ...
    if(huart == &huart2)
    {
        if(sendCompleteSign2 == FALSE || len == 0)
        {
            return FALSE;
        }
        sendCompleteSign2 = FALSE;
    }
    status = HAL_UART_Transmit_DMA(huart, (unsigned char*)buf, len);
